...
modis_band_list = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7']
...
def main(ifile, shp_file, center_lonlat, salz, sala,
            ...
            band_need=['all'], path_out_radi=None, path_out_6s=None):
    '''
    @description: 主程序
    @ifile {str}: L1B swath文件 
...
def stack_rgb(ifile_250m, ifile_500m):
    raster1 = gdal.Open(ifile_250m)
    raster2 = gdal.Open(ifile_500m)
    xsize = raster1.GetRasterBand(1).ReadAsArray().shape[1]
    ysize = raster1.GetRasterBand(1).ReadAsArray().shape[0]
    nbands = 4
    if '250M.tif' in ifile_250m:
        raster_fn_out = ifile_250m.replace('250M.tif', 'rgb.tif')
    else:
        print('无法识别的文件名: %s' % os.path.split(ifile_250m)[1])
        return(0)
    driver = gdal.GetDriverByName('GTiff')
    target_ds = driver.Create(raster_fn_out, xsize, ysize, nbands, gdal.GDT_Float64)
    target_ds.SetGeoTransform(raster1.GetGeoTransform())
    target_ds.SetProjection(raster1.GetProjectionRef())
    target_ds.GetRasterBand(1).WriteArray(raster1.GetRasterBand(1).ReadAsArray()) # 655nm
    target_ds.GetRasterBand(2).WriteArray(raster1.GetRasterBand(2).ReadAsArray()) # 865nm
    target_ds.GetRasterBand(3).WriteArray(raster2.GetRasterBand(3).ReadAsArray()) # 1240nm
    target_ds.GetRasterBand(4).WriteArray(raster2.GetRasterBand(4).ReadAsArray()) # 1640nm(损坏)
    target_ds = None
    raster1 = None
    raster2 = None

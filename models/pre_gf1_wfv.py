...
'''
@Author: cuiqiyuan
@Description: 高分1号WFV传感器数据(16m)预处理
...
# 辐射定标系数表
SCALES = {
    'WFV1_2018': [0.1824, 0.1546, 0.1270, 0.1344],
    'WFV2_2018': [0.1851, 0.1538, 0.1231, 0.1314],
    'WFV3_2018': [0.1894, 0.1728, 0.1343, 0.1373],
    'WFV4_2018': [0.1866, 0.1599, 0.1307, 0.1251],
    'WFV1_latest': [0.1824, 0.1546, 0.1270, 0.1344],
    'WFV2_latest': [0.1851, 0.1538, 0.1231, 0.1314],
    'WFV3_latest': [0.1894, 0.1728, 0.1343, 0.1373],
    'WFV4_latest': [0.1866, 0.1599, 0.1307, 0.1251]
}
...
def main(ifile, shp_file=None, cut_range=None, aerotype=1,
    ...
    for item in file_list:
        if 'WFV' in item and '.tiff' in item:
            mss_name = item
            break
    print('数据预处理...')
    # 通过文件名获取传感器日期
    ...
    if date_str[0:4] == '2018' and 'WFV1' in path_name:
        scales = SCALES['WFV1_2011']
    elif date_str[0:4] == '2018' and 'WFV2' in path_name:
        scales = SCALES['WFV2_2018']
    elif date_str[0:4] == '2018' and 'WFV3' in path_name:
        scales = SCALES['WFV3_2018']
    elif date_str[0:4] == '2018' and 'WFV4' in path_name:
        scales = SCALES['WFV4_2018']
    elif 'WFV1' in path_name:
        scales = SCALES['WFV1_latest']
    elif 'WFV2' in path_name:
        scales = SCALES['WFV2_latest']
    elif 'WFV3' in path_name:
        scales = SCALES['WFV3_latest']
    elif 'WFV4' in path_name:
        scales = SCALES['WFV4_latest']
    else:
        print('无法识别的传感器类型!')
        return(0)
    month = date_str[4:6]
    ...
    # 获取太阳和卫星的方位信息
    for item in file_list:
        if 'WFV' in item and '.xml' in item:
            xml_name = item
            break
    ...
    name_out = '%s_%s_16_L2_%s_%s_%s.tiff' % (satellite_code, sensor_code, date_str, nrow, ncolm)
    ...
    # 拷贝RPB文件
    for item in file_list:
        if 'WFV' in item and '.rpb' in item:
            rpb_name = item
            break

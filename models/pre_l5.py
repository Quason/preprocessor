    ...
    with open(MTLfile, 'r') as fp:
        for line in fp.readlines():
            if 'RADIANCE_MULT_BAND_1 ' in line:
                ...
                K1_band6 = float(line.split(' ')[-1])
            elif 'K2_CONSTANT_BAND_6' in line:
                K2_band6 = float(line.split(' ')[-1])
    coef = {}
    coef['mult'] = [mult_B1, mult_B2, mult_B3, mult_B4, mult_B5, mult_B6, mult_B7]
    coef['add'] = [add_B1, add_B2, add_B3, add_B4, add_B5, add_B6, add_B7]
    coef['sun'] = [sun_zenith, sun_azimuth]
    coef['date'] = dateStr
    coef['location'] = [np.mean([lon_ul, lon_ur, lon_ll, lon_lr]),
                        np.mean([lat_ul, lat_ur, lat_ll, lat_lr])]
    coef['K1_B6'] = K1_band6
    coef['K2_B6'] = K2_band6
    return(coef)
...
def radi_arms_corr(raster_data, MTLfile, altitude, visibility, type_aero, wave_index):
    ...
    if wave_index != 5:
        ...
        atms_corr_data = mtl_coef['K2_B6'] / np.log(mtl_coef['K1_B6']/radi_cali_data+1)

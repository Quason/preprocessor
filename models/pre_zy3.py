...
'''
@Author: cuiqiyuan
@Description: 资源3号MUX传感器数据(6m)预处理
...
# 辐射定标系数表
SCALES = {
    'MUX_2018': [0.244, 0.215, 0.175, 0.189],
    'MUX_latest': [0.244, 0.215, 0.175, 0.189]
}
...
def main(ifile, shp_file=None, cut_range=None, aerotype=1,
    ...
    for item in file_list:
        if 'MUX' in item and item[-5:]=='.tiff':
            mss_name = item
            break
    print('数据预处理...')
    # 通过文件名获取传感器日期
    ...
    if date_str[0:4] == '2018' and 'MUX' in path_name:
        scales = SCALES['MUX_2018']
    elif 'MUX' in path_name:
        scales = SCALES['MUX_latest']
    else:
        print('无法识别的传感器类型!')
        return(0)
    month = date_str[4:6]
    ...
    # 获取太阳和卫星的方位信息
    for item in file_list:
        if 'MUX' in item and '.xml' in item and not('rpb' in item):
            xml_name = item
            break
    ...
    name_out = '%s_%s_6_L2_%s_%s_%s.tiff' % (satellite_code, sensor_code, date_str, nrow, ncolm)
    ...
    # 拷贝RPB文件和xml文件
    for item in file_list:
        if 'MUX' in item and '.rpb' in item:
            rpb_name = item
            break
